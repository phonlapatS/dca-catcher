...
ต่อไปเมื่อหุ้นถึงเป้าหมาย บอทจะแจ้งเตือนคุณผ่านทาง **{mode_text}** ครับ"""
    ...
    async def send_premarket_watchlist_digest(self):
        ...
        for symbol in us_symbols:
            ...
            if not grade_result:
                # Cache miss, call AI
                grade_result = await asyncio.to_thread(self.grader.grade, enriched[symbol], risk_profile="ทั่วไป (Aggregated)")
                # Save to cache
                try:
                    cache_meta = {
                        "targets": getattr(grade_result, 'buy_targets', []),
    ...
    async def broadcast_scan(self, market: str=None):
        ...
        for (symbol, rp), users in symbol_risk_users.items():
            ...
            if not result:
                ...
                result = await asyncio.to_thread(self.grader.grade, signal, risk_profile=rp)
